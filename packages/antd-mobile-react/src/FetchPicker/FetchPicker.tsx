...
export const FetchPicker: React.FC<FetchPickerProps> = ({
  ...
  const { label: labelFieldName = 'label', value: valueFieldName = 'value' } =
    fieldNames || {};
      ...
      if (multiple) {
        if (labelInValue) {
          return options.filter((option) => {
            return some(
              ...
              (v) => get(option, valueFieldName) === get(v, 'value'),
            );
          });
        } else {
          return options.filter((option) => {
        ...
        if (labelInValue) {
          return options.find((option) =>
            isEqual(get(option, valueFieldName), get(value, 'value')),
          );
        } else {
          return options.find(
            (option) => get(option, valueFieldName) === value,
          );
        }
      }
    },
  ...
  const handleSelected = useCallback(
    (item: any) => {
      ...
      if (multiple) {
        if (labelInValue) {
          return some(
            internalValue as LabeledValue[],
            (v) => get(v, 'value') === get(item, valueFieldName),
          );
        } else {
          return (internalValue as (string | number)[])?.includes(
            get(item, valueFieldName),
          );
        }
      } else {
        if (labelInValue) {
  ...
  const handleFetch = useCallback(
    (params: { page: number; size: number; wd?: string }) => {
      if (!fetch) {
        setOptions([]);
        setHasMore(false);
        return;
      ...
      fetch(params)
        .then((res) => {
          ...
          setOptions((prev) =>
            params.page === 1 ? nextOptions : [...prev, ...nextOptions],
          );
  ...
  const header = fullScreen ? (
    <>
      <Space>
        {backable && (
          <div className={`${cls}-head-icon`} onClick={onBack}>
            {backIcon || <LeftOutline />}
          </div>
        )}
        {closable && (
          <div className={`${cls}-head-icon`} onClick={onClose}>
    ...
    <>
      <div className={`${cls}-head-button`} onClick={onClose}>
        {cancelText}
      </div>
      <div className={`${cls}-head-title`}>{title}</div>
      <div className={`${cls}-head-button`} onClick={handleOnOk}>
        {okText}
      </div>
    </>
  );
        ...
        <div className={cls}>
          ...
          {showSearch && (
            <div className={`${cls}-search-bar`}>
              <Input
                className={`${cls}-search-bar-input`}
                prefix={
          ...
          >
            ...
            {isEmpty(options) &&
              !loading &&
              (empty || <div className={`${cls}-empty`}>暂无数据</div>)}
            {options?.map((item, index) => {
              const selected = handleSelected(item);
              return (
                <div
                  className={classNames(`${cls}-item`, `${cls}-item-option`, {
                    [`${cls}-item-option-selected`]: selected,
                  })}
                  key={`${get(item, valueFieldName) ?? index}`}
                  onClick={() => {
                    handleItemClick(item);
                  }}
                >
                  <div className={`${cls}-item-option-content`}>
                    {optionRender?.(item, selected) ||
                      get(item, labelFieldName)}
                  </div>
                  {multiple && selected && (
                    <div className={`${cls}-item-option-state`}>
                      <CheckOutline />
                    </div>
                  )}
                </div>
            ...
            {!isEmpty(options) && loading && (
              <div className={`${cls}-loading-more`}>
                加载更多
                <DotLoading />
              </div>
            )}
